...
const DataQualityDashboard: React.FC<DataQualityDashboardProps> = ({
  ...
  const runAnalysis = async () => {
    try {
      ...
      } else if (sftpCredentialId && sftpRemotePath) {
        const download = await api.downloadFromSFTP({
          credential_id: sftpCredentialId,
          remote_path: sftpRemotePath,
        });
        if (!download.success || !download.content) {
          throw new Error(download.error || 'Failed to download file from SFTP');
        }
        response = await api.analyzeDataContent(download.content, filename || 'data.csv');
      } else {
      ...
      if (response.success && response.analysis) {
        ...
        const mappedResult: QualityResult = {
          ...
          completeness_score: Math.round(
            qualityScore.completeness_score || analysis.completeness_score || 0
          ),
          validity_score: Math.round(qualityScore.validity_score || analysis.validity_score || 0),
          consistency_score: Math.round(
            qualityScore.consistency_score || analysis.consistency_score || 0
          ),
          uniqueness_score: Math.round(
            qualityScore.accuracy_score || analysis.uniqueness_score || 0
          ),
                    ...
                    {field.issues.length > 0 && (
                      <div className="mt-2 flex gap-2 flex-wrap">
                        {field.issues.map((issue, i) => (
                          <span
                            key={i}
                            className="text-xs bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 px-2 py-1 rounded"
                          >
                            {issue}
                          </span>
                        ))}
