...
export const APIKeyManager: React.FC<APIKeyManagerProps> = ({
  ...
  const deleteKey = (keyId: string) => {
    ...
    if (deletedKey && !updatedKeys.some((k) => k.provider === deletedKey.provider)) {
      const storeKey =
        deletedKey.provider === 'google' ? 'apiKeys.gemini' : `apiKeys.${deletedKey.provider}`;
                ...
                <Card key={key.id} className="p-4 border-primary bg-primary/5">
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => saveEdit(key.id)}>
                        Save
                      </Button>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        ...
                        <span
                          className={`text-xs px-2 py-1 rounded-full ${getStatusColor(key.status)}`}
                        >
                          {key.status}
                        </span>
                      </div>
