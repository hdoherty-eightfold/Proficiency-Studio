    ...
    const saveVersion = async () => {
        ...
        try {
            ...
            const configData = {
                ...
                proficiency_levels: DEFAULT_PROFICIENCY_LEVELS.map(({ color, ...level }) => level)
            };
            ...
            toast({ title: 'Save failed', description: err instanceof Error ? err.message : 'Failed to save prompt', variant: 'destructive' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const testPrompt = async () => {
        ...
        try {
            ...
            const response = await api.post<Record<string, any>>('/api/proficiency/assess', {
                ...
                skills: testInputs.skill_name ? [{ name: testInputs.skill_name, category: 'test' }] : [{ name: 'Test Skill', category: 'test' }],
            ...
            setTestResult({
                ...
                input: testInputs,
                output: result.reasoning || result.output || result.assessment || JSON.stringify(result, null, 2),
            ...
            if (selectedVersion) {
                ...
                const newAvgLatency = Math.round((currentStats.avg_latency_ms * currentStats.uses + latency) / newUses);
                setSelectedVersion(prev => prev ? {
                    ...prev,
                    stats: {
                        ...prev.stats,
...
Provide your analysis:`,
            ...
            toast({ title: 'Failed to create template', description: err instanceof Error ? err.message : 'Unknown error', variant: 'destructive' });
        } finally {
            setCreating(false);
        }
    };
    ...
    const getCategoryColor = (category: string) => {
        switch (category) {
            ...
            case 'extraction': return 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300';
            case 'mapping': return 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300';
            case 'review': return 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300';
        ...
        <div className="h-full flex flex-col p-6 space-y-6 overflow-y-auto bg-gray-50 dark:bg-gray-900">
            ...
            <div className="grid grid-cols-12 gap-6 flex-1 min-h-0">
                ...
                <div className="col-span-3 space-y-2 overflow-y-auto">
                    <h3 className="font-semibold text-sm text-gray-500 uppercase tracking-wide mb-3">Templates</h3>
                    {initialLoading ? (
                            ...
                            <div className="flex items-center gap-4 mb-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => {
                                            const version = selectedTemplate.versions.find(v => v.id === e.target.value);
                                    ...
                                    <div className="mt-4">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {extractVariables(editContent).map(v => (
                                                <span key={v} className="px-2 py-1 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded text-xs font-mono">
                                                    {`{{${v}}}`}
                                                </span>
                                            ))}
                                        </div>
                                ...
                                <Card className="mt-4 bg-white dark:bg-gray-800">
                                    ...
                                    <CardContent className="space-y-4">
                                        <div className="grid grid-cols-2 gap-4">
                                            {extractVariables(editContent).map(v => (
                                                <div key={v}>
                                                    <label className="text-xs font-medium text-gray-500 mb-1 block">{v}</label>
                                                    <input
                                                        type="text"
                                                        value={testInputs[v] || ''}
                                        ...
                                        <Button onClick={testPrompt} disabled={testing} className="w-full">
                                            {testing ? <RefreshCw className="w-4 h-4 mr-2 animate-spin" /> : <Play className="w-4 h-4 mr-2" />}
                                            Run Test
                                        </Button>
                                        {testResult && (
