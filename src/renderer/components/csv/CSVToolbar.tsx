        ...
        <>
            <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <CardDescription>Double-click any cell to edit. Changes are saved immediately.</CardDescription>
                    </div>
                    <div className="flex gap-2">
                        ...
                        <Button onClick={onRefresh} variant="ghost" size="icon" title="Refresh data" aria-label="Refresh data">
            ...
            <div className="flex items-center gap-4 mb-4 flex-wrap px-6">
                <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium ${
                    ...
                    <span>{validationSummary.invalidRows === 0 ? 'All Valid' : `${validationSummary.invalidRows} Issues`}</span>
                </div>

                <div className="flex items-center gap-3 text-sm">
                    <span className="text-gray-600 dark:text-gray-400">
                        <strong className="text-gray-900 dark:text-white">{validationSummary.totalRows}</strong> total
                    </span>
                ...
                {Object.keys(validationSummary.fieldIssues).length > 0 && (
                    <div className="flex items-center gap-2 text-xs text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 px-2 py-1 rounded">
                        Missing: {Object.entries(validationSummary.fieldIssues).map(([field, count], i) => (
                            <React.Fragment key={field}>
                                {i > 0 && ', '}
                                <span><strong>{field}</strong> ({count})</span>
                            </React.Fragment>
                        ))}
                    </div>
                )}
