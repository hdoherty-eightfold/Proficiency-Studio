  ...
  return (
    <div className="space-y-3">
      <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
        <div className="overflow-x-hidden max-h-[500px] overflow-y-auto scrollbar-thin">
          <table className="border-collapse w-full" aria-label="CSV data editor">
            <thead className="bg-gray-100 dark:bg-gray-900 sticky top-0 z-20">
              <tr>
                <th
                  scope="col"
                  className="px-2 py-2 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 border-r border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 sticky left-0 z-30"
                ...
                <th
                  scope="col"
                  className="px-2 py-2 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 border-r border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 sticky left-[40px] z-30"
                ...
                <th
                  scope="col"
                  className="px-2 py-2 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 border-r border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 sticky left-[90px] z-30"
                  style={{ width: '40px', minWidth: '40px' }}
                >
                ...
                {visibleColumns.map((col, idx) => (
                  <th
                    ...
                    style={{
                      width: `${calculatedWidths[col] || 150}px`,
                      minWidth: `${calculatedWidths[col] || 150}px`,
                    }}
                  >
                    <div className="flex items-center gap-1">
                      {col}
                      {requiredFields.includes(col) && <span className="text-red-500">*</span>}
                    </div>
                    <div
            ...
            <tbody>
              {paginatedData.map(({ row, originalIndex }) => {
                ...
                const rowHasError = Array.from(validationErrors.keys()).some((key) =>
                  key.startsWith(`${originalIndex}-`)
                );
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm">
          <span className="text-gray-500 dark:text-gray-400">
            ...
            {Math.min(currentPage * ROWS_PER_PAGE, filteredData.length)} of {filteredData.length}{' '}
            rows
          </span>
